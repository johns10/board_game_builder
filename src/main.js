...
document.addEventListener('DOMContentLoaded', () => {
    ...
    tileSystem = new TileSystem(gameConfig);
...
function movePlayer(player, spaces) {
    ...
    animateMovement(player, newPosition, () => {
        // Handle tile effects
        const tileEffects = tileSystem.handleTileEffects(player, newPosition, gameState, {
            updateStatus: updateStatusMessage,
            updateUI: updateUI,
            movePlayer: movePlayer,
            animateMovement: animateMovement,
            nextTurn: nextTurn,
            handlePlayerLoss: handlePlayerLoss,
            setRollButtonState: (disabled) => { rollButton.disabled = disabled; },
            handleComputerTurn: handleComputerTurn
        });

        if (tileEffects.length > 0) {
            // Execute money tile effect first if present
            const moneyEffect = tileEffects.find(effect => effect.type === 'money');
            if (moneyEffect) {
                moneyEffect.execute();

                setTimeout(() => {
                    // Execute special tile effect if present
                    const specialEffect = tileEffects.find(effect => effect.type === 'special');
                    if (specialEffect) {
